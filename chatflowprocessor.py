...
def validate_and_get_absolute_zip_path(zip_file_path: str) -> str:
    ...
    Returns:
    Path: The absolute path object of the zip file.
...
def get_conversation_messages(conversation: Dict) -> List[Dict[str, str]]:
    ...
    while currentNode:
        ...
        if message and isinstance(message.get("content"), dict):
            content = message["content"]
            author_role = message.get("author", {}).get("role")

            if author_role in ["assistant", "user", "tool"]:
